namespace Protogame
{
    ...
    public class RawAudioLoadStrategy : ILoadStrategy
    {
        ...
        {
            var file = new FileInfo(Path.Combine(path, (noTranslate ? name : name.Replace('.', Path.DirectorySeparatorChar)) + ".wav"));
            if (file.Exists)
            {
        ...
        {
            yield return Path.Combine(path, (noTranslate ? name : name.Replace('.', Path.DirectorySeparatorChar)) + ".wav");
        }
    }
}