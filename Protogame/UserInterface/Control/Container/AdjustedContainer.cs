...
namespace Protogame
{
    public class AdjustedContainer : IContainer
    {
        private readonly Point _originalAbsolutePosition;
        ...
        public AdjustedContainer(Point originalAbsolutePosition)
        {
            _originalAbsolutePosition = originalAbsolutePosition;
        }
        ...
        public IEnumerable<KeyValuePair<IContainer, Rectangle>> ChildrenWithLayouts(Rectangle layout)
        {
            for (var i = 0; i < _positions.Count; i++)
            {
                ...
                var relativePosition = new Rectangle(layout.X + (position.X - _originalAbsolutePosition.X), layout.Y + (position.Y - _originalAbsolutePosition.Y), position.Width,
                    position.Height);
